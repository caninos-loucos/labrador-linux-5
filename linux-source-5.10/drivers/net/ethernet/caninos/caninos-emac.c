...
/*
 * Caninos K5 Ethernet MAC driver
 *
...
static inline void owl_emac_reg_write(struct owl_emac_priv *priv, u32 reg, u32 data)
{
	writel(data, priv->base + reg);
}

static inline u32 owl_emac_reg_update(struct owl_emac_priv *priv, u32 reg, u32 mask, u32 val)
{
	u32 data, old_val;
...
static inline void owl_emac_reg_set(struct owl_emac_priv *priv, u32 reg, u32 bits)
{
	owl_emac_reg_update(priv, reg, bits, bits);
}

static inline void owl_emac_reg_clear(struct owl_emac_priv *priv, u32 reg, u32 bits)
{
	owl_emac_reg_update(priv, reg, bits, 0);
}

static inline struct device *owl_emac_get_dev(struct owl_emac_priv *priv)
{
...
static const struct net_device_ops owl_emac_netdev_ops = {
	...
	.ndo_do_ioctl = owl_emac_ndo_eth_ioctl,
	.ndo_tx_timeout = owl_emac_ndo_tx_timeout,
	.ndo_get_stats = owl_emac_ndo_get_stats,
};

static void owl_emac_ethtool_get_drvinfo(struct net_device *dev, struct ethtool_drvinfo *info)
{
...
static __maybe_unused int caninos_emac_suspend(struct device *dev)
{
...
static __maybe_unused int caninos_emac_resume(struct device *dev)
{
	struct net_device *netdev = dev_get_drvdata(dev);
	struct owl_emac_priv *priv = netdev_priv(netdev);
	int ret;
...
static void caninos_emac_phy_hw_reset(struct owl_emac_priv *priv)
{
...
static int caninos_emac_phy_init(struct net_device *netdev)
{
	struct owl_emac_priv *priv = netdev_priv(netdev);
	struct device *dev = owl_emac_get_dev(priv);
	struct phy_device *phy;
	
	caninos_emac_phy_hw_reset(priv);
	
	phy = of_phy_get_and_connect(netdev, dev->of_node, owl_emac_adjust_link);
...
static int caninos_emac_get_gpios(struct owl_emac_priv *priv)
{
	struct device *dev = owl_emac_get_dev(priv);
	int reset_gpio, power_gpio, ret;
	
	reset_gpio = of_get_named_gpio(dev->of_node, "phy-reset-gpio", 0);
	...
	power_gpio = of_get_named_gpio(dev->of_node, "phy-power-gpio", 0);
	...
	ret = devm_gpio_request(dev, reset_gpio, "phy_reset");
	...
	ret = devm_gpio_request(dev, power_gpio, "phy_power");
...
static int caninos_emac_set_pin_mux(struct owl_emac_priv *priv)
{
	struct device *dev = owl_emac_get_dev(priv);
	int ret;
	
	priv->pinctrl = pinctrl_get_select_default(dev);
	
	if (IS_ERR(priv->pinctrl)) {
		ret = PTR_ERR(priv->pinctrl);
		priv->pinctrl = NULL;
		dev_err(dev, "unable to select pinctrl default state %d\n", ret);
		return ret;
	}
	return 0;
}

static int caninos_emac_clk_set_rate(struct owl_emac_priv *priv)
{
	struct device *dev = owl_emac_get_dev(priv);
	int ret;
...
static void caninos_emac_get_mac_addr(struct net_device *netdev)
{
	struct device *dev = netdev->dev.parent;
	u8 addr[ETH_ALEN];
	
	if (!eth_platform_get_mac_address(dev, addr)) {
		eth_hw_addr_set(netdev, addr);
	}
	
	if (is_valid_ether_addr(netdev->dev_addr)) {
		dev_info(dev, "using MAC address from DT %pM\n", netdev->dev_addr);
		return;
	}
...
static void caninos_emac_clk_disable_unprepare(void *data)
{
...
static int caninos_emac_probe(struct platform_device *pdev)
{
	...
	ret = caninos_emac_set_pin_mux(priv);
	
	if (ret) {
		return ret;
	}
	
	ret = caninos_emac_get_gpios(priv);
	...
	ret = devm_request_irq(dev, netdev->irq, owl_emac_handle_irq, IRQF_SHARED, netdev->name, netdev);
	...
	ret = devm_add_action_or_reset(dev, caninos_emac_clk_disable_unprepare, priv);
	
	if (ret) {
		return ret;
	}
	
	ret = caninos_emac_clk_set_rate(priv);
	...
	if (IS_ERR(priv->reset)) {
		return dev_err_probe(dev, PTR_ERR(priv->reset), "failed to get reset control");
	}
	
	caninos_emac_get_mac_addr(netdev);
	...
	ret = caninos_emac_phy_init(netdev);
...
static int caninos_emac_remove(struct platform_device *pdev)
{
...
static const struct of_device_id caninos_emac_of_match[] = {
	{ .compatible = "caninos,ethernet-k5" },
	{ /* sentinel */ }
};
MODULE_DEVICE_TABLE(of, caninos_emac_of_match);

static SIMPLE_DEV_PM_OPS(caninos_emac_pm_ops, caninos_emac_suspend, caninos_emac_resume);

static struct platform_driver caninos_emac_driver = {
	.driver = {
		.name = DRIVER_NAME,
		.of_match_table = caninos_emac_of_match,
		.pm = &caninos_emac_pm_ops,
	},
	.probe = caninos_emac_probe,
	.remove = caninos_emac_remove,
};
module_platform_driver(caninos_emac_driver);

MODULE_AUTHOR("Caninos Loucos, ITEX/LSITEC");
